...
def _add_constraints(ilp, lpvars):
    ...
    for (g1, g2), path_var in lpvars.path_vars.iteritems():
        ...
        ilp += lpSum([lpvars.dup_vars[node] for node in path]) - path_var >= 0
        ilp += lpSum([lpvars.dup_vars[node] for node in path]) - len(path) * path_var <= 0
    ...
    for gleaves in gleaves_by_species.values():
        for g1, g2 in combination(gleaves, 2):
            path = dlclp_utils.find_path_without_lca(g1, g2)
            ilp += lpSum([lpvars.dup_vars[node] for node in path]) >= 1
    ...
    for snode, local_top in lpvars.loss_vars:
        ...
        ilp += lpSum(local_paths) - local_loss - local_helper <= len(local_bottoms) - 1
    ...
    for snode, local_top in lpvars.helper_vars:
        ...
        ilp += lpSum(prev_paths) + local_helper <= len(prev_nodes)
        ilp += lpSum(prev_paths) + len(lpvars.top_nodes[snode]) * local_helper >= len(prev_nodes)
    ...
    for snode, local_top in lpvars.coal_vars:
        ...
        ilp += lpSum(prev_paths) + local_coal <= len(prev_nodes)
        ilp += lpSum(prev_paths) + len(lpvars.top_nodes[snode]) * local_coal >= len(prev_nodes)
    ...
    for g2 in lpvars.gtree.preorder():
        ...
        ilp += lpvars.r_vars[g2] >= lpSum(other_delta_vars_in_same_species) - 1
