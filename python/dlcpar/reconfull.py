...
def dlc_recon(tree, stree, gene2species,
              ...
              max_srecon=1, max_srecon_depth=2, delay=True,
    ...
    reconer = DLCRecon(tree, stree, gene2species,
                       ...
                       implied=implied,
                       max_srecon=max_srecon, max_srecon_depth=max_srecon_depth, delay=delay,
...
class DLCRecon(object):
    ...
    def __init__(self, gtree, stree, gene2species,
        ...
        assert (max_srecon > 0) and (max_srecon_depth > 0)
        self.max_srecon = max_srecon
        self.max_srecon_depth = max_srecon_depth
        self.delay = delay
...
##        self.log.log("\n\n")
        ...
        # setup for multiple species maps        
        opt_cost = util.INF
        i_srecon = 0
        next_name = self.gtree.nextname
        srecon_enum = phylo.enum_recon(self.gtree, self.stree,
                                       #recon=srecon, #events=sevents,
                                       depth=self.max_srecon_depth,
                                       gene2species=self.gene2species)
        self.srecon, self.sevents = srecon_enum.next()
        
        while True:
            # start log
            self.log.start("species map %d" % i_srecon)
                           
            # reset
            self.gtree.nextname = next_name
            ...
            added_spec, added_dup, added_delay = reconlib.add_implied_nodes(self.gtree, self.stree, self.srecon, self.sevents, delay=self.delay)
            ...
            # infer locus map
            cost = self._infer_locus_map(assert_single=(i_srecon==0))
            self.log.log("\n\n")        
            ...
            # store if new optimal solution
            if cost < opt_cost:
                opt_cost = cost
                opt_srecon = copy.copy(self.srecon)
                opt_lrecon = self.lrecon
                opt_order = self.order

            # remove implied nodes
            reconlib.remove_implied_nodes(self.gtree, added_spec, added_dup, added_delay, self.srecon, self.sevents)

            # stop log
            self.log.stop()

            # try next species map
            try:
                i_srecon += 1
                if i_srecon == self.max_srecon:
                    break
                self.srecon, self.sevents = srecon_enum.next()
        ...
        labeled_recon = reconlib.LabeledRecon(opt_srecon, opt_lrecon, opt_order)
    ...
    def _infer_locus_map(self, assert_single=True):
        ...
        self.log.log("\n\n")
        opt_cost = self._infer_opt_locus_map(locus_maps, subtrees)
##        opt_cost = self._infer_trivial_locus_map()
        self.log.stop()
...
##                            self.log.log()
    ...
    def _infer_trivial_locus_map(self):
        """For debugging only - may not return valid locus map"""
    ...
    def _infer_opt_locus_map(self, locus_maps, subtrees):
        ...
        opt_cost = cost_to_go
        self.log.log("")
        self.log.log("Optimal cost: %g" % opt_cost)
        self.log.log("")
