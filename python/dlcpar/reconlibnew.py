...
def remove_spec_from_dup_nodes(node, tree, recon=None, events=None):
    """Remove added nodes"""
    
    assert events[node] == "spec"
    events[node.parent] = "spec"
...
def remove_implied_spec_nodes(tree, added_spec, added_dup, recon=None, events=None):
    """Remove added nodes"""
    
    for node in added_dup:
        remove_spec_from_dup_nodes(node, tree, recon, events)
    phylo.remove_implied_spec_nodes(tree, added_spec, recon, events)
...
def remove_delay_nodes(node, tree, recon=None, events=None):
    """Remove delay nodes"""
    
    assert events[node] == "delay"
    phylo.remove_spec_node(node, tree, recon, events)
...
def remove_implied_delay_nodes(tree, added_delay, recon=None, events=None):
    """Remove added nodes"""
    
    for node in added_delay:
        remove_delay_nodes(node, tree, recon, events)
...
def remove_implied_nodes(tree,
                         added_spec, added_dup, added_delay=None,
                         recon=None, events=None):
    """Wrapper to remove implied speciation nodes and delay nodes"""
    
    if added_delay is not None:
        remove_implied_delay_nodes(tree, added_delay, recon=recon, events=events)
    remove_implied_spec_nodes(tree, added_spec, added_dup, recon=recon, events=events)
    ...
    for node in tree:
        if events[node] == "spec":
            ...
            for schild in snode.children:
                ...
                if len(nodes2) > 0:
                    assert (not assert_single) or (assert_single and len(nodes2) == 1), (node, nodes2)
                    node2 = nodes2[0]
